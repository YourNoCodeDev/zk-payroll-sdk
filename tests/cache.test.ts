...
describe("ZKProofGenerator with cache", () => {
	...
	it("different witnesses produce separately cached entries", async () => {
		...
		await ZKProofGenerator.generateProof(
			{ recipient: "b", amount: 2n },
			cache,
		);

		expect(await cache.has(witnessToCacheKey({ recipient: "a", amount: 1n }))).toBe(
			true,
		);
		expect(await cache.has(witnessToCacheKey({ recipient: "b", amount: 2n }))).toBe(
			true,
		);
	});
});
